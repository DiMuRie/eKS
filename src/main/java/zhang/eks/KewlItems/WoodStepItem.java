...
public class WoodStepItem extends Item {

	private Block block;

    public WoodStepItem(WoodSteps block)
    {
        this.block = block;
...
/*
    public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ,IBlockState state)
    {
        IBlockState iblockstate = worldIn.getBlockState(pos);
        Block block = iblockstate.getBlock();
        if (block == EKSBlockLoader.WoodSteps && ((Integer)iblockstate.getValue(WoodSteps.LAYERS)).intValue() < 1)
        {
